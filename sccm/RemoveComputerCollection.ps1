<#
    Objetivo: Remove Computer em uma Collection
    Version: 1.0
    Autor: Marcelo Galdino Pereira
#>

Write-Host -ForegroundColor Green "========================================= Remove Computer em uma Collection ========================================="

#Servidor SCCM
$servidorSCCM = Read-Host "Informe o nome do servidor de SCCM"

$nomeCollection = Read-Host "Informe o nome da Collection para ser removido no SCCM"

# Solicitando o arquivo com os servidores
$entradaServidores = Get-Content (Read-Host "Insira o caminho e o nome do txt com os servidores para remover da Collection: Ex: c:\temp\servidores.txt")
...
try {
    Invoke-Command -ComputerName $servidorSCCM -ScriptBlock {
        ...
        Write-Host "Iniciando a remocao..."
        try{
            foreach ($server in $using:entradaServidores) {
                Remove-CMDeviceCollectionDirectMembershipRule -CollectionName $using:nomeCollection `
                -ResourceID ((Get-CMDevice -Name $server).ResourceID) -Force -ErrorAction Stop
            }
        }catch{ Write-Host -ForegroundColor Yellow "Servidor $server nao estava na Collection"}
    } # fim do Invoke
